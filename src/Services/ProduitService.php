...
class ProduitService {
    ...
    #region Recupere un produit via Id et Titre
    /**
     * @param int    $id
     * @param string $titre
     *
     * @return Produit
     */
    public function findProduitIdTitre(int $id, string $titre): Produit
    {
        if (!$this->exist($id)){
            throw new NotFoundHttpException("Ce Produit n'existe pas !");
        }
        return $this->produitRepository->findOneBy(['id'=>$id, 'titre'=>$titre]);
    }
    #endregion
...
//== Administration
    ...
    #region Controle l'existence d'1 Produit via Titre et image
    /**
     * Controle l'existence d'une Catégorie de Produit
     *
     * @param int $id
     *
     * @return array
     */
    public function allProduitByCat(int $id):array
    {
        return $this->produitRepository->findBy(['categorie' =>$id]);
    }
    #endregion
//== Fin Administration
}